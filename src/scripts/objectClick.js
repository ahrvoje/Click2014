    ...
    var getMousePos = function (event) {
        ...
        highlightGroup = function (group) {
            ...
            for (i = 0; i < group.length; i++) {
                drawField(group[i][0], group[i][1], colors.playColors[game.currentPosition[group[i][0]][group[i][1]] - 1]);
            }
        },
        ...
        drawAllFields = function () {
            ...
            } else if (game.status === game.StatusPlay || game.status === game.StatusOver || game.status === game.StatusAutoPlay) {
            ...
            if (game.status === game.StatusOver || game.status === game.StatusAutoPlay) {
        ...
        updateTimer = function () {
            ...
            if (game.status !== game.StatusAutoPlay) {
                currentTime = (new Date().getTime() - game.startTime) / 1000.0;
            } else {
